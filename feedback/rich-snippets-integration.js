...
(function() {
    ...
    // Get feedback data from database using ApiHelper (handles proxy vs direct automatically)
    async function getDatabaseFeedbackData() {
        try {
            // Check if ApiHelper is available
            if (!window.ApiHelper) {
                console.warn('ApiHelper not available for rich snippets');
                return null;
            }

            // Initialize ApiHelper (will auto-detect proxy vs direct connection)
            await window.ApiHelper.initialize();

            // Use ApiHelper for all database operations
            const data = await window.ApiHelper.callDatabase(
                'select',
            ...
            // Sort by created_at desc and limit to 100 records
            const sortedData = data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 100);

            // Calculate aggregate statistics
            const totalRating = sortedData.reduce((sum, item) => sum + parseFloat(item.average_rating), 0);
            const avgRating = totalRating / sortedData.length;

            log(`Loaded ${sortedData.length} feedback submissions from database`);

            return {
                history: sortedData,
                storedRating: avgRating,
                storedCount: sortedData.length,
                source: 'database'
            };
        } catch (error) {
